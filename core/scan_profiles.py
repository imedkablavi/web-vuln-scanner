...
from .scan_strategies import STRATEGIES, apply_strategy
...
def apply_profile(config: Dict[str, Any], profile: str) -> Dict[str, Any]:
    ...
    # Safety and coverage are intentionally orthogonal. The profile decides
    # whether active/browser/auth features may run. The strategy only changes
    # discovery breadth and passive coverage budgets.
    selected_strategy = str(
        result["scanner"].get("strategy", "balanced") or "balanced"
    ).strip().lower()
    result = apply_strategy(result, selected_strategy)
    result["scanner"]["profile"] = profile
    return result
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Write one built-in safety profile plus discovery strategy to YAML."
    )
