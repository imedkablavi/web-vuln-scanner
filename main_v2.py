...
def load_config(config_path):
    ...
    if not isinstance(config, dict) or not isinstance(config.get("scanner"), dict):
        typer.echo("Invalid config: missing top-level 'scanner' mapping", err=True)
        raise typer.Exit(code=2)
    return config
...
def validate_threads(config):
    try:
        ...
        typer.echo(f"Invalid thread configuration: {exc}", err=True)
...
def validate_target(target: str):
    ...
    if not parsed.hostname:
        raise typer.BadParameter("target must include a hostname")
    if parsed.username is not None or parsed.password is not None:
        raise typer.BadParameter("credentials in target URLs are not allowed")
    return parsed


def _mark_partial(scan_meta, message):
    if scan_meta.get("status") != "failed":
        scan_meta["status"] = "partial"
    if message and message not in scan_meta["warnings"]:
        scan_meta["warnings"].append(message)


def _collect_inventory_urls(
    target,
    surfaces,
    crawler=None,
    browser_report=None,
    ...
    if crawler is not None:
        urls.extend(
            url
            for url in getattr(crawler, "visited", set())
...
def _build_browser_login_summary(auth_summary):
    actor_states = (
        auth_summary.get("actor_states", {}) if isinstance(auth_summary, dict) else {}
    )
    browser_actors = []
    browser_only = []
    ...
    return {
        ...
        "successful_handoff": [
            actor_id for actor_id in browser_actors if actor_id not in browser_only
        ],
    }
...
async def run_scan_async(target, config, swagger_url, graphql_url, output_dir=None):
    ...
    artifact_store = ArtifactStore(
        config["scanner"].get("output", {}).get("directory", "reports")
    )

    req_manager = RequestManager(config["scanner"])
    req_manager.attach_event_bus(event_bus)
    auth_session_manager = req_manager.auth_session_manager or AuthSessionManager(
        config["scanner"], requester=req_manager
    )
    ...
    browser_auth_engine = BrowserAuthEngine(
        config["scanner"],
    ...
    scan_meta = {
        ...
        "notes": [],
        "skipped_checks": [],
        "profile": config["scanner"].get("profile", "custom"),
        "execution": {
            "collection_methods": [],
    ...
    if target.startswith(
        ...
        scan_meta["notes"].append(
            "Results were produced against a local or mock target; they validate "
            "scanner behavior, not a third-party asset."
        )
    ...
    scan_meta["execution"]["layers"]["browser_login"] = _build_browser_login_summary(
    ...
    if auth_session_summary.get("stats", {}).get("failed_login"):
        _mark_partial(
            scan_meta,
            "One or more auth actors failed login or could not be proven ready.",
        )
    if scan_meta["execution"]["layers"]["browser_login"].get(
        "browser_authenticated_only"
    ):
        _mark_partial(
            scan_meta,
            "One or more browser-driven logins succeeded visually but could not "
            "be handed off to the HTTP layer.",
        )
    if (
        config["scanner"].get("auth_verification", {}).get("enabled")
        and not auth_session_summary.get("ready")
    ):
        scan_meta["skipped_checks"].append(
            "Some auth verification scenarios were skipped because not enough "
            "actors reached an authenticated ready state."
        )

    browser = None
    try:
        browser_enabled = config["scanner"].get("browser_enabled", False)
        ...
        if browser_enabled:
            try:
                try:
                    ...
                    logger.warning(
                        f"Playwright not available ({exc}). Falling back to HTTP crawler."
                    )
                ...
                if browser_enabled:
                    ...
                    if getattr(browser, "errors", []):
                        _mark_partial(
                            scan_meta,
                            "Browser-assisted crawl encountered errors.",
                        )
                    logger.info(
                        f"Browser finished. Found {len(surfaces)} surfaces."
                    )
            except Exception as exc:
                logger.warning(
                    f"Browser engine failed, falling back to crawler: {exc}"
                )
        ...
        if crawler_enabled:
            ...
            auth_crawl_cfg = (
                config["scanner"]
                .get("auth_verification", {})
                .get("authenticated_crawl", {})
            )
            if auth_crawl_cfg.get("enabled", False):
                ...
                for actor_id in actor_ids:
                    ...
                    if actor is None or not auth_session_manager.is_actor_ready(actor):
                        scan_meta["skipped_checks"].append(
                            f"Authenticated crawl skipped for actor {actor_id} "
                            "because the session was not ready."
                        )
                        continue
        ...
        scan_meta["execution"]["layers"]["auth_verification"] = (
            auth_harness.get_summary()
        )
        scan_meta["execution"]["layers"]["auth_sessions"] = (
            auth_session_manager.get_summary()
        )
        if not auth_harness.is_ready():
            scan_meta["skipped_checks"].append(
                "Auth-aware verification was not run because fewer than two valid "
                "actors were configured."
            )
        ...
        if scanner.last_run_stats.get("errors"):
            _mark_partial(
                scan_meta,
                "Active scanner encountered plugin execution errors.",
            )

        if workflow_runner.enabled():
            try:
                workflow_instances, workflow_findings = (
                    await workflow_runner.run_configured(target)
                )
                scan_meta["execution"]["layers"]["workflow_execution"] = (
                    workflow_runner.get_summary()
                )
                if any(
                    instance.workflow_status
                    in {"partial", "failed", "indeterminate", "blocked_auth"}
                    for instance in workflow_instances
                ):
                    _mark_partial(
                        scan_meta,
                        "One or more workflow executions completed only partially "
                        "or indeterminately.",
                    )
            except Exception as exc:
                scan_meta["execution"]["layers"]["workflow_execution"] = (
        ...
        if workflow_runner.enabled():
            replay_verified_only = bool(
                config["scanner"]
                .get("workflows", {})
                .get("replay_verified_only", True)
            )
            for instance in workflow_instances:
                if not instance.definition.replayable:
                    continue
                if (
                    replay_verified_only
                    and instance.final_verdict.verification_status != "verified"
                ):
                    continue
                replay_result = await replay_engine.replay_workflow_execution(
                    instance,
                    workflow_runner,
                    target=target,
                )
                ...
                for finding in workflow_findings:
                    if finding.workflow_execution_id != instance.execution_id:
                        continue
                    finding.workflow_replay_status = replay_result["record"][
                        "replay_status"
                    ]
        ...
        if browser is not None and auth_harness.is_ready():
            try:
                ...
                _mark_partial(
                    scan_meta,
                    f"Browser actor artifact capture failed: {exc}",
                )
        ...
        scan_meta["execution"]["layers"]["browser_login"] = (
            _build_browser_login_summary(
                scan_meta["execution"]["layers"]["auth_sessions"]
            )
        )
        scan_meta["execution"]["layers"]["rbac_policy"] = rbac_verifier.get_summary()
        ...
        logger.error(f"Scan failed with error: {exc}")
        ...
        if browser is not None:
            try:
                ...
                logger.warning(f"Browser cleanup failed: {cleanup_exc}")
...
def scan(
    ...
    profile: str = typer.Option(
        "passive",
        "--profile",
        help="Safety profile: passive, safe-active, or full-authorized",
    ),
    ...
    debug: bool = typer.Option(
        False,
        "--debug",
        help="Enable debug counters logging",
    ),
):
    """Evidence-first web vulnerability scanner for explicitly authorized targets."""
    ...
    scope_cfg = config["scanner"].setdefault("scope", {})
    scope_cfg.setdefault("allowlist", [])
    scope_cfg.setdefault("include_domains", [])
    if domain not in scope_cfg["allowlist"]:
        scope_cfg["allowlist"].append(domain)
    if domain not in scope_cfg["include_domains"]:
        scope_cfg["include_domains"].append(domain)
    scope_cfg.setdefault("resolve_dns", True)
    try:
        ip = ipaddress.ip_address(hostname)
        if ip.is_private or ip.is_loopback:
            scope_cfg["allow_private"] = True
    except ValueError:
    ...
    config["scanner"]["debug"] = bool(
        debug or config["scanner"].get("debug", False)
    )
    ...
    try:
        config["scanner"].setdefault("output", {})
        config["scanner"]["output"]["directory"] = output_dir
        findings, scan_meta = asyncio.run(
            run_scan_async(
                target,
                config,
                swagger,
        ...
        scan_meta["status"] = "failed"
        scan_meta.setdefault("errors", []).append(str(exc))
    finally:
        ...
        typer.echo(f"\n{_status_header(scan_meta.get('status', 'completed'))}")
        typer.echo(f"Profile: {scan_meta.get('profile', profile)}")
        ...
        if scan_meta.get("execution", {}).get("surfaces_discovered", 0) == 0:
            ...
            typer.echo(
                ...
                f"suspected={status_counts.get('suspected', 0)}, "
    ...
    raise typer.Exit(
        code=scan_exit_code(scan_meta.get("status", "completed"), len(findings))
    )


if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1].lower() == "scan":
        # Support both `python main_v2.py scan <target>` and
        # `python main_v2.py <target>`.
        sys.argv.pop(1)
    app()
